...
def parse_excel(file) -> dict:
    ...
    if 'RAW_GA 변환' in xl:
        ...
        # 별빛약국의 포스터(테스트2) → 약국 자체 제작 QR로 분류
        if '유입 매체' in df.columns:
            mask = (df['약국'].astype(str).str.contains('별빛', na=False)) & \
                   (df['유입 매체'].astype(str) == '포스터(테스트2)')
            df.loc[mask, '유입 매체'] = '약국 자체 제작 QR'
        result['ga'] = df
...
<div style="display:flex; align-items:center; gap:12px; margin-bottom:8px;">
    ...
    <div>
        <div class="main-title" style="font-size:1.6rem; font-weight:800; color:#FFFFFF; line-height:1.2">메디QR 주간 인사이트</div>
...
with tabs[0]:
    ...
    if 'ga' in main_data:
        ...
        # 모바일: 2×2 그리드 / 데스크톱: 4컬럼 — CSS grid로 처리
        st.markdown(f"""
<style>
.metric-grid {{
    display: grid;
...
""", unsafe_allow_html=True)
        ...
        fig.update_layout(height=280, plot_bgcolor='white', paper_bgcolor='white',
                          ...
                          font=dict(size=11))
        fig.update_yaxes(title_text="방문자", secondary_y=False, gridcolor='#F1F5F9', title_font_size=11)
        fig.update_yaxes(title_text="스캔", secondary_y=True, title_font_size=11)
