...
with tabs[3]:
    ...
    if not ai_enabled:
        ...
        curr_w = weeks[-1]
        curr_df = df[df['유입 일자'].dt.to_period('W').astype(str) == curr_w]
        ...
        media_summary = df.groupby('유입 매체').agg(총사용자=('총 사용자 수', 'sum'), 바코드스캔=('바코드 이벤트 횟수', 'sum')).to_dict() if '유입 매체' in df.columns else {}
...
을 실무자가 바로 활용할 수 있는 형태로 분석해주세요."""
        ...
        if compare_data and 'ga' in compare_data:
            ...
            if st.button("🔄 전주 대비 변화 분석", type="primary"):
                ...
                curr_pharm = set(df['약국'].unique())
                prev_pharm = set(prev_df['약국'].unique())
...
【변화】
- 방문자 증감: {curr_total - prev_total:+,}명 ({(curr_total-prev_total)/prev_total*100:+.1f}%)
- 스캔 증감: {curr_scan - prev_scan:+,}회 ({(curr_scan-prev_scan)/prev_scan*100:+.1f}% if prev_scan > 0 else 'N/A')
